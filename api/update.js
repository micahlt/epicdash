...
module.exports = async (req, res) => {
  let q = req.query;
  if (q.username && q.token && q.id && q.title && q.url && q.labels) {
        ...
        (err, results) => {
          ...
          if (results.length > 0) {
            ...
            if (tokens.includes(q.token) && Boolean(results[0].admin)) {
              connection.query(
                `UPDATE sites SET title = ${connection.escape(
                  decodeURIComponent(q.title)
                )}, url = ${connection.escape(
                  decodeURIComponent(q.url)
                )}, labels = ${connection.escape(
                  decodeURIComponent(q.labels)
                )} WHERE id = ${connection.escape(q.id)}`,
                (err) => {
                  if (!err) {
